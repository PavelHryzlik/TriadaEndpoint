...
{
    public class JsonLdDumpHandler : BaseResultsHandler
    {
        ...
        public JsonLdDumpHandler(TextWriter output, Uri contextUri, bool prettyprint, bool closeOutput)
        {
        ...
        protected override bool HandleResultInternal(SparqlResult result)
        {
            foreach (String var in result.Variables)
            {
                ...
                if (value.Contains("contract") || value.Contains("amendment") || value.Contains("attachment"))
                {
                    ...
                    var document = (JObject)jsonLdCompacted["@graph"].Children().FirstOrDefault(n =>n["@type"].ToString() == "Contract" || n["@type"].ToString() == "Attachment" || n["@type"].ToString() == "Amendment");
